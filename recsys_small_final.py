...
# Utility functions to load in images
...
# Loading the Lottie Animation file
def load_lottiefile(filepath: str):
    with open(filepath, "r") as f:
        return json.load(f)

lottie_loading = load_lottiefile("res/movie2.json")

# Loading in the Logo's
img = Image.open('res/nn.png')
st.set_page_config(page_title="New Netflix", page_icon=img,layout="wide",initial_sidebar_state="expanded")
heading_logo=get_img_with_href('res/heading.png',"")
st.markdown(heading_logo, unsafe_allow_html=True)
hide_menu_style = """
        <style>
...
# Creating two columns to display the text and the animation side by side
...
with open("res/movies_list_small.txt", "rb") as fp:
...
mv_list= pd.read_csv('res/mv_list_small.csv',header=None)
dict_corr_mat=load('res/corr_mat_small.npz')
...
mv_csv=pd.read_csv('res/movies.csv')
links_csv=pd.read_csv('res/links.csv')
...
for k in movie_rec:
    ...
    c1,c2=st.columns(2)
    with c1:
        if(k=='Untitled Spider-Man Reboot (2017)'): # Correcting a Data issue
            k='Spider-Man: Homecoming (2017)'
        at((k,genres ,qq, ))

    imdb_html = get_img_with_href('res/imdb (1).png', f_imdb)
